...
# qPCR validation of IR events using Delta CT method to calculate fold change of intron over exon expression

### IRX2
irx2_ct <- readxl::read_xls("/camp/home/ziffo/home/projects/astrocyte-fractionation-bulk-rnaseq/sample-details/irx2_qpcr.xls", sheet = "CT_values") #%>% mutate(mutation = str_split_fixed(condition, "_", 2)[,1], feature = str_split_fixed(condition, "_", 2)[,2]) 
irx2_ct.df <- irx2_ct %>% select(IRX2_intron,IRX2_exon) %>% as.data.frame() 
group_var <- irx2_ct %>% pull(condition)
irx2_qpcr <- pcr_analyze(irx2_ct.df,
            group_var = group_var,
            reference_gene = "IRX2_exon", # control gene
            reference_group = "ctrl") # ,plot = TRUE) # control group
irx2_qpcr
irx2_intron_expression <- ggplot(data=irx2_qpcr, aes(x=group, y=relative_expression, fill=group)) + geom_bar(stat="identity", width=0.8, color = "black", position=position_dodge()) + 
  geom_errorbar(aes(ymin=lower, ymax=upper), width=.2, position=position_dodge(.9)) +
  theme_classic() + theme(legend.position="none", axis.text=element_text(size=10)) +
  #facet_grid(. ~ mutation) + 
  scale_fill_manual(values=c("white", "black")) +  xlab("") + ylab( expression(fold~change~normalised~intron~expression)  )
irx2_intron_expression


# Spliced transcript expression
irx2_ct <- readxl::read_xls("/camp/home/ziffo/home/projects/astrocyte-fractionation-bulk-rnaseq/sample-details/irx2_qpcr.xls", sheet = "CT_values") #%>% mutate(mutation = str_split_fixed(condition, "_", 2)[,1], feature = str_split_fixed(condition, "_", 2)[,2]) 
irx2_ct.df <- irx2_ct %>% select(IRX2_constitutive, GAPDH) %>% as.data.frame() 
group_var <- irx2_ct %>% pull(condition)
irx2_qpcr <- pcr_analyze(irx2_ct.df,
            group_var = group_var,
            reference_gene = "GAPDH", # control gene
            reference_group = "ctrl") # ,plot = TRUE) # control group
irx2_qpcr
irx2_spliced_transcript <- ggplot(data=irx2_qpcr, aes(x=group, y=relative_expression, fill=group)) + geom_bar(stat="identity", width=0.8, color = "black", position=position_dodge()) + 
  geom_errorbar(aes(ymin=lower, ymax=upper), width=.2, position=position_dodge(.9)) +
  theme_classic() + theme(legend.position="none", axis.text=element_text(size=10)) +
  #facet_grid(. ~ mutation) + 
  scale_fill_manual(values=c("white", "black")) +  xlab("") + ylab( expression(fold~change~spliced~transcript~expression)  )
irx2_spliced_transcript

ggarrange(irx2_intron_expression, irx2_spliced_transcript)
...
# irx2_ct.df <- irx2_ct %>% mutate(mutation = str_split_fixed(condition, "_", 2)[,1], feature = str_split_fixed(condition, "_", 2)[,2])  %>% as.data.frame() %>% select(IRX2, GAPDH, condition, mutation, feature)
irx2_ct.df <- irx2_ct %>% as.data.frame() %>% select(IRX2, GAPDH)

group_var <- irx2_ct %>% mutate(feature = str_split_fixed(condition, "_", 2)[,2]) %>% pull(feature)
...
irx2_qpcr <- pcr_analyze(irx2_ct.df,
            group_var = group_var,
            # reference_gene = "GAPDH", # control gene
            reference_group = "exon",
            method = 'delta_ct') # ,plot = TRUE) # control group
irx2 <- irx2_qpcr %>% select(relative_expression, mutation, feature)
  pivot_wider(names_from = feature, values_from = relative_expression) %>% mutate(fold_change = intron / exon)
irx2_plot <- ggplot(data=irx2, aes(x=mutation, y=fold_change, fill=mutation)) + geom_bar(stat="identity", width=0.8, color = "black", position=position_dodge()) + 
  # geom_errorbar(aes(ymin=lower, ymax=upper), width=.2, position=position_dodge(.9)) + 
  theme_classic() + theme(legend.position="none", axis.text=element_text(size=10)) +
  #facet_grid(. ~ mutation) + 
  scale_fill_manual(values=c("white", "black")) +  xlab("") + ylab( expression(fold~change~intron/exon)  )
irx2_plot


irx2_test <- irx2 %>% dplyr::select(relative_expression, feature) %>% t_test(relative_expression ~ feature, var.equal = TRUE) %>% add_significance() %>% mutate(y.position = 4.2)
irx2_plot <- ggplot(data=irx2, aes(x=feature, y=relative_expression, fill=feature)) + geom_bar(stat="identity", width=0.8, color = "black", position=position_dodge()) + 
  geom_errorbar(aes(ymin=lower, ymax=upper), width=.2, position=position_dodge(.9)) + theme_classic() + theme(legend.position="none", axis.text=element_text(size=10)) +
  facet_grid(. ~ mutation) + scale_fill_manual(values=c("white", "black")) +  xlab("") + ylab( expression(IRX2)  )
irx2_plot
...
absolute.cytoplasmic <- VCP.cyt.ctrl.filt %>% full_join(VCP.cyt.mutant.filt, by = c("Name.Coords", "coords", "ensID", "gene_name", "Chr", "Start", "End", "Strand", "gc_content", "intron_length", "fraction"))  %>% 
  mutate(retained = case_when(ctrl_retained == "retained" | vcp_retained == "retained" ~ "retained", TRUE ~ "spliced"))
...
# Absolute IR characteristics
VCP.nuc.ctrl.filt <- ac_nuc_ctrl.IR %>% dplyr::select(Name.Coords, coords, ensID, gene_name, Chr, Start, End, Strand, gc_content, intron_length, IRratio, reliable, VCP.nuc.ctrl.retained = retained) %>% filter(reliable == "reliable")
VCP.nuc.mutant.filt <- ac_nuc_vcp.IR %>% dplyr::select(Name.Coords, coords, ensID, gene_name, Chr, Start, End, Strand, IRratio,reliable, VCP.nuc.mutant.retained = retained, gc_content, intron_length) %>% filter(reliable == "reliable") 
VCP.cyt.ctrl.filt <- ac_cyt_ctrl.IR %>% dplyr::select(Name.Coords, coords, ensID, gene_name, Chr, Start, End, Strand, gc_content, intron_length, IRratio, reliable, VCP.cyt.ctrl.retained = retained) %>% filter(reliable == "reliable")
VCP.cyt.mutant.filt <- ac_cyt_vcp.IR %>% dplyr::select(Name.Coords, coords, ensID, gene_name, Chr, Start, End, Strand, IRratio,reliable, VCP.cyt.mutant.retained = retained, gc_content, intron_length) %>% filter(reliable == "reliable") 
ctrl.vcp.nuc.cyt.IR <- VCP.nuc.ctrl.filt %>% full_join(VCP.nuc.mutant.filt, by = c("Name.Coords", "coords", "ensID", "gene_name", "Chr", "Start", "End", "Strand", "gc_content", "intron_length")) %>%
  ...
  mutate(retained = case_when(VCP.nuc.ctrl.filt == "retained" | VCP.nuc.mutant.filt == "retained" | SOD1.ctrl.retained == "retained" | SOD1.mutant.retained == "retained" | C9ORF72.ctrl.retained == "retained" | C9ORF72.mutant.retained == "retained" | A1.ctrl.retained == "retained" | A1.mutant.retained == "retained" ~ "retained", TRUE ~ "spliced"))
