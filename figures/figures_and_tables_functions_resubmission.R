...
all.reactivity.go <- c(focal.adhesion.go, collagen.formation.go, extracellular.matrix.go, go.pathways.msigdb$GO_RESPONSE_TO_WOUNDING,
                       ...
                       go.pathways.msigdb$GO_PHAGOCYTOSIS, glutamate.uptake.go, 
...
curated_profiles <- 
  function(gprofiles = IRevents_gprofiles_curated, colours = c("firebrick2", "dodgerblue3")){
    gprofiles %>% 
      ...
      theme(strip.background = element_blank(), legend.position = c(0.8, 0.2), strip.text.y = element_text(size=8, face = "bold",angle = 0), axis.text.y= element_text(size = 8)) +
  ...
  function(data = boxplot.lfc.merged, groups = "category", continuous = "log2FoldChange", flip = "yes", tip.length = 0.01, labels = "gene_name", colours = 3, stat.y.position = 1.00, label_number = 10, stats.test = "t_test", expected = 0, width = 0.2, dashed.line = 0){
    ...
    if(flip == "yes"){
      ...
      ggplot(data2, aes(x = group, y= lfc)) + 
        ...
        geom_hline(yintercept =dashed.line, linetype = 2) + coord_flip() +
        geom_text_repel(data = top_n(data2, label_number, abs(lfc)), aes(x = group, y = lfc, label = label), fontface = "italic", colour = "black", min.segment.length = 0, box.padding = 0.5, point.padding = 0, size=4, max.overlaps = Inf) +
