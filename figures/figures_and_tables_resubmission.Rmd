...
TARDBP.clip.differential.sina_plot <- sinaplot(data = differential.3datasets.IR, groups = "p0.05_reliable_IRdirection", continuous = "iclip_score.normalised", dashed.line = "NA", stat.y.position = 0.11, stats.test = "t_test") +  ylab(label = expression(log[10]~TARDBP~iCLIP~binding) ) + xlab("ALS vs CTRL astrocytes") + scale_y_log10() + theme(axis.text=element_text(size=10)) +
  geom_segment(aes(x = 2.55, xend = 2.55, y= 0.0001, yend= 0.9), arrow=arrow(length=unit(0.4,"cm")), color = "black") +
  annotate("text", x = 2.45, y = 0.01, label = "TDP43 Binding to Intron Up", colour = "black", size = 2.8, angle = 90) 
...
# GSEA of all RBPs
filter(dge_res_ac_who_vcp_vs_ctrl, gene_name %in% go.pathways.msigdb$GO_RNA_BINDING, log2FoldChange > 0) %>% nrow # 94
# NMD sinaplot
dge_res_ac_who_vcp_vs_ctrl$condition <- "RBP GSEA"
rbp_who_sina_plot <- sinaplot.one.sample(data =  filter(dge_res_ac_who_vcp_vs_ctrl, gene_name %in% go.pathways.msigdb$GO_RNA_BINDING), groups = "condition", continuous = "log2FoldChange", labels = "gene_name", colours = 2,  label_number = 0, stat.y.position = 1, width = 0.1, flip = "no", stats.test = "t-test") + ylab(label = expression( RBP~GSEA~LFC~VCP:CTRL )) + xlab("") + ylim(c(-1,1))  + theme(axis.text=element_text(size=10)) +
  geom_segment(aes(x = 1.5, xend = 1.5, y= 0.1, yend= 1), arrow=arrow(length=unit(0.2,"cm")), color = "black") +  geom_segment(aes(x = 1.5, xend = 1.5, y= -0.1, yend= -1),arrow=arrow(length=unit(0.2,"cm")), color = "black") +
  annotate("text", x = 1.35, y = 0.6, label = "VCP RBP Up", colour = "black", size = 2.7, angle = 90) +  annotate("text", x = 1.35, y = -0.5, label = "VCP RBP Down", colour = "black", size = 2.7, angle = 90)+
 geom_text_repel(data = filter(dge_res_ac_who_vcp_vs_ctrl, gene_name == "TARDBP"), aes(x = condition, y = log2FoldChange, label = gene_name), fontface = "italic", colour = "black", min.segment.length = 0, box.padding = 3, 
                 point.padding = 0, size = 2.8, max.overlaps = Inf,    nudge_y= -0.15, direction = "x") 
rbp_who_sina_plot
# [1] "Stats test used =  t-test"
# # A tibble: 1 x 9
#   group    .y.   group1 group2         n statistic        p p.signif y.position
#   <chr>    <chr> <chr>  <chr>      <int>     <dbl>    <dbl> <chr>         <dbl>
# 1 RBP GSEA lfc   1      null model  1903    511197 1.59e-14 ****              1

# RBP GSEA
geneList <- dge_res_ac_who_vcp_vs_ctrl %>% mutate(log2FoldChange = case_when(log2FoldChange == "NA" ~ 0, TRUE ~ log2FoldChange)) %>% pull(log2FoldChange)
...
RBP_genes.list <- list("RBP" = dge_res_ac_who_vcp_vs_ctrl$gene_name[dge_res_ac_who_vcp_vs_ctrl$gene_name %in% go.pathways.msigdb$GO_RNA_BINDING])
fgseaRes <- fgsea(pathways = RBP_genes.list, stats = geneList)
fgseaRes
#    pathway pval padj    log2err        ES       NES size                               leadingEdge
# 1:     RBP    1    1 0.01335463 -0.148988 -0.617624 1681 DCPS,SLFN11,RALYL,CCDC59,PABPC1L,BZW1,...


# dge_res_ac_who_vcp_vs_ctrl %>% filter(gene_name == "TARDBP")
# # # A tibble: 1 x 8
# #   ensID           baseMean log2FoldChange lfcSE   stat pvalue  padj gene_name
# #   <chr>              <dbl>          <dbl> <dbl>  <dbl>  <dbl> <dbl> <chr>    
# # 1 ENSG00000120948    8432.         -0.258 0.507 -0.508  0.611  1.00 TARDBP  
# dge_res_ac_nuc_vcp_vs_ctrl %>% filter(gene_name == "TARDBP")
# # # A tibble: 1 x 8
# #   ensID           baseMean log2FoldChange lfcSE   stat pvalue  padj gene_name
# #   <chr>              <dbl>          <dbl> <dbl>  <dbl>  <dbl> <dbl> <chr>    
# # 1 ENSG00000120948    8559.         -0.228 0.379 -0.601  0.548  1.00 TARDBP   
# dge_res_ac_cyt_vcp_vs_ctrl %>% filter(gene_name == "TARDBP")
# # # A tibble: 1 x 8
# #   ensID           baseMean log2FoldChange lfcSE   stat pvalue  padj gene_name
# #   <chr>              <dbl>          <dbl> <dbl>  <dbl>  <dbl> <dbl> <chr>    
# # 1 ENSG00000120948    9406.        -0.0660 0.468 -0.141  0.888  1.00 TARDBP   
# 
# # Cyto / Nuc in CTRL
# # dge_res_ac_ctrl_cyt_vs_nuc %>% filter(gene_name == "TARDBP")
# #   ensID           baseMean log2FoldChange lfcSE   stat pvalue  padj gene_name
# #   <chr>              <dbl>          <dbl> <dbl>  <dbl>  <dbl> <dbl> <chr>    
# # 1 ENSG00000120948    9816.         -0.347 0.489 -0.710  0.478 0.689 TARDBP   
#   
# # Cyto / Nuc in VCP
# # dge_res_ac_vcp_cyt_vs_nuc %>% filter(gene_name == "TARDBP")
# #   ensID           baseMean log2FoldChange lfcSE   stat pvalue  padj gene_name
# #   <chr>              <dbl>          <dbl> <dbl>  <dbl>  <dbl> <dbl> <chr>    
# # 1 ENSG00000120948    8219.         -0.231 0.348 -0.665  0.506 0.648 TARDBP  
#   
#  
...
fig3.merged.new <- ggarrange(
  ggarrange(a1_tdp43_sina_plot, a1_tdp43_dge_ir_sina_plot, TARDBP.clip.differential.sina_plot, rbp_who_sina_plot, nrow = 1, ncol = 4, widths = c(0.8,1.2, 0.8, 0.5), labels = c("D", "E", "F", "G")), # IR sina & IR GE sina
  ggarrange(ac_bar_a1_scatter, ac_tdp_ko_scatter, nrow = 1, ncol = 2, widths = c(1,1), labels = c("H", "I")), # IR & GE scatter
  ggarrange(ac_bar_a1_go_curated, ac_tdp_go_curated, TRAP.translationUP.go_curated, nrow = 1, ncol = 3, widths = c(1,1,1), labels = c("J", "K", "L")), # GO terms
  NULL,
  ggarrange(
    ...
    nrow = 3, ncol = 1, heights = c(1,1,0.9), labels = c("M", "N", "O")), # liddelow heatmap
...
## Delta IR ratio vs Log2FoldChange
...
vcp_sod1_c9orf_sina_plot <- sinaplot.one.sample(data = vcp_sod1_c9orf, groups = "mutation", continuous = "IR.l2fc", labels = "gene_name", label_number = 0, stat.y.position = 5, width = 0.1, flip = "no", colours = 4) + ylab(label = expression(IR~ratio~Log2FoldChange~ALS:CTRL) ) + xlab("") + ylim(c(-5,5)) +
  geom_segment(aes(x = 3.4, xend = 3.4, y= 1, yend= 5), arrow=arrow(length=unit(0.4,"cm")), color = "black") +
  geom_segment(aes(x = 3.4, xend = 3.4, y= -1, yend= -5),arrow=arrow(length=unit(0.4,"cm")), color = "black") +
  annotate("text", x = 3.3, y = 3, label = "ALS IR Up", colour = "black", size = 3, angle = 90) + annotate("text", x = 3.3, y = -3, label = "ALS IR Down", colour = "black", size = 3, angle = 90)
vcp_sod1_c9orf_sina_plot
```
