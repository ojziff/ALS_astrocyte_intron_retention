...
# Import pooled replicate quantified IRFinder Output for each condition i.e. not differentially compared (IRFinder-IR-dir.txt or if not stranded, then IRFinder-IR-nondir.txt or )
...
# Import already compared IRFinder output:
...
# import gene expression results (using HTSeq counts in intersection-strict mode)
...
# import mass spec results
mass_spec_ac_vcp_vs_ctrl_stp <- read_excel("/camp/home/ziffo/home/projects/astrocyte-meta-analysis/expression/mass-spectrometry/ac_vcp_fractions_mass_spectrometry_doaa.xlsx", sheet = "All Protein data")
...
dds <- DESeqDataSetFromHTSeqCount(sampleTable = sampleTable, directory = htseqdir, design = ~ condition) # create DESeqDataSet with this design - Import htseq .tab tables
dds <- DESeq(dds) # run DESeq2
vsd_ac_who_vcp_vs_ctrl <- vst(dds, blind=FALSE) # VST transformation
colnames(vsd_ac_who_vcp_vs_ctrl) <- colnames(vsd_ac_who_vcp_vs_ctrl) %>% gsub("whole_", "", .) %>% gsub("5", "2", .) %>% gsub("cb1e", "R191Q", .) %>% gsub("glia", "R155C", .) # Labels for mutants are based on VCP mutation. R155C mutant = GLIA. R191Q mutant = CB1E.
pcaData_astrocyte <- plotPCA(vsd_ac_who_vcp_vs_ctrl, intgroup=c("sample", "condition", "mutation", "cellline","fraction", "patient_fraction", "patient"), returnData=TRUE)
percentVar_astrocyte <- round(100 * attr(pcaData_astrocyte, "percentVar"))
ac_who_pca <- ggplot(pcaData_astrocyte, aes(PC1, PC2, color=mutation)) + geom_text_repel(aes(label=patient), size = 3) + scale_colour_manual( values = c("dodgerblue", "firebrick2") ) +  theme_bw() +  
  theme(panel.grid = element_blank(), legend.title = element_blank(), legend.text=element_text(size=8), legend.position = "right", legend.box = "vertical", legend.spacing = unit(0.2, 'cm')) + geom_point(size=2) + 
  xlab(paste0("PC1: ",percentVar_astrocyte[1],"% variance")) +  ylab(paste0("PC2: ",percentVar_astrocyte[2],"% variance")) + coord_fixed()
sampleDists <- dist(t(assay(vsd_ac_who_vcp_vs_ctrl)))
hc <- hclust(sampleDists, method = "ward.D2")
a <- c("dodgerblue", "dodgerblue", "firebrick2","firebrick2")
...
ggsave(ac_who_pca_dendrogram, filename = "/camp/home/ziffo/home/projects/astrocyte-meta-analysis/splicing/IRFinder/irlib/figures/Fig1B_C.ac_who_pca_dendrogram.png", height = 2.5, width = 5)
```
